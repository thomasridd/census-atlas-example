<script>
    ...
    import {bbox, center, distance} from "@turf/turf";
    ...
    import NearestTable from "./NearestTable.svelte";
    ...
    function initialise() {
        getTopo(ladtopo.url, ladtopo.layer)
            .then((geo) => {
                ...
                lad_nearest_neighbours = getLadNearestNeighbours(geo)
    ...
    function getLadNearestNeighbours(geo) {
        let lad_nearest_neighbours = {}
        let lad_coords = geo.features.map(f => {
            return {
                'code': f.properties[ladtopo.code],
                'name': f.properties[ladtopo.name],
                'center': center(f)
        ...
        lad_coords.forEach(lad => {
                lad_nearest_neighbours[lad.code] = lad_coords.map(alt => {
                    return {
                        'code': alt.code,
                        'name': alt.name,
                        'distance': distance(lad.center, alt.center)
                    }
                }).sort((a, b) => a.distance < b.distance ? -1 : (a.distance > b.distance ? 1 : 0))
            }
        )

        return lad_nearest_neighbours
    }
    ...
    function loadData() {
        console.log(`loading data for item ${selectItem.code}...`);
        loading = true;
...
<Panel>
    <div class="grid">
        ...
        {#if indicators && selectItem}
            ...
            <div class="grid__col col-12@m">
                {#if active.lad.hovered || active.lad.selected}
                    <NearestTable selected={active.lad.hovered ? active.lad.hovered : active.lad.highlighted ? active.lad.highlighted : active.lad.selected}
                                  nearestLookup={lad_nearest_neighbours}
                                  data={selectData}/>
                {/if}
            </div>
            <div class="grid__col col-12@m">
